""" Python Implementation of reg.py for windows
    ...
    (I am also now aware that argparse libraries exist)
...
""" 
    Notes: 
        Modified due to issues with the original code submission
        New code makes use of winreg module
        Fixes problems with previous submission where full file paths needed to be used
        (Can now use hive-names instead of full file paths)
        
    
        Also added in some new functions to aid in the use of winreg objects
        Now using factory patterns to deal with object creation
        Removed redundant code from last submission


        This code HAS been tested and works on real registry files
        I have tested all general use cases that I can think of, and none seemed to have 
        any issues. 
        
        Eg Has been tested with

        python reg.py ADD HKEY_CURRENT_USER\Printers\Connections /v test /d test

        python reg.py QUERY HKEY_CURRENT_USER\Software 

        python reg.py QUERY "HKEY_CURRENT_USER\Control Panel\International\Geo" /v Nation

        python reg.py DELETE "HKEY_CURRENT_USER\Control Panel\Internation\Geo" /v Nation


        MINOR ISSUES:
            When adding new values to keys, program fails if the input data is not
            of type winreg.REG_SZ. 
            Mainly because I had issues converting the input data into other 
            weirder data types (eg REG_BINARY). 
